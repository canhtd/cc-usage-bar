...
R4. **Session state machine** (waitingForBanner → waitingForPrompt → waitingForResult → capturing → stopped) with per-query IDs, timeouts (30s), idle-settle (1.5s), SIGWINCH nudge to force full Ink redraw; detect first-run/onboarding screens → `needsSetup` with instruction "Run `claude` in Terminal once and log in". Detect `claude` not found (exit 127) → clear message. **One `claude` process per query — no session reuse.** Claude Code answers `/usage` from a cache it builds once per process, so a session kept alive across refreshes replays stale numbers indefinitely (observed: menu bar frozen at 35%/3% for 30 min while the CLI itself reported 65%/6%). Every refresh launches a child, captures, and tears it down immediately — on success and on failure alike. Overlapping fetches for one profile are still rejected (F3). Kill child + close PTY on quit; no zombie processes (waitpid).
